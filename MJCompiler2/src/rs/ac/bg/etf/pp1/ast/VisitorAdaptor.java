// generated with ast extension for cup
// version 0.8
// 26/7/2021 20:29:37


package rs.ac.bg.etf.pp1.ast;

public abstract class VisitorAdaptor implements Visitor { 

    public void visit(ExprWithBrackets ExprWithBrackets) { }
    public void visit(Mulop Mulop) { }
    public void visit(LabelList LabelList) { }
    public void visit(AddopExpr AddopExpr) { }
    public void visit(Relop Relop) { }
    public void visit(TermList TermList) { }
    public void visit(FormParameters FormParameters) { }
    public void visit(ProgramDeclList ProgramDeclList) { }
    public void visit(CaseSt CaseSt) { }
    public void visit(StatementList StatementList) { }
    public void visit(VarPart VarPart) { }
    public void visit(ProgramDecl ProgramDecl) { }
    public void visit(Addop Addop) { }
    public void visit(ConstPart ConstPart) { }
    public void visit(Factor Factor) { }
    public void visit(CondTerm CondTerm) { }
    public void visit(DummyNum DummyNum) { }
    public void visit(ConditionFactOp ConditionFactOp) { }
    public void visit(ConstList ConstList) { }
    public void visit(Designator Designator) { }
    public void visit(Term Term) { }
    public void visit(FormParsList FormParsList) { }
    public void visit(Condition Condition) { }
    public void visit(ExprZeroOrOne ExprZeroOrOne) { }
    public void visit(CaseList CaseList) { }
    public void visit(LabeledStatement LabeledStatement) { }
    public void visit(ActParsList ActParsList) { }
    public void visit(ElsePart ElsePart) { }
    public void visit(Brackets Brackets) { }
    public void visit(VarDeclList VarDeclList) { }
    public void visit(Expr Expr) { }
    public void visit(VarDeclarationList VarDeclarationList) { }
    public void visit(Expr1 Expr1) { }
    public void visit(ActPars ActPars) { }
    public void visit(MethodTypeName MethodTypeName) { }
    public void visit(DesignatorStatement DesignatorStatement) { }
    public void visit(Const Const) { }
    public void visit(ActualPars ActualPars) { }
    public void visit(NumConstOneOrZero NumConstOneOrZero) { }
    public void visit(Statement Statement) { }
    public void visit(CondFact CondFact) { }
    public void visit(MethodDeclList MethodDeclList) { }
    public void visit(MinusOneOrZero MinusOneOrZero) { }
    public void visit(DesignatorOpPart DesignatorOpPart) { }
    public void visit(ExtendsPart ExtendsPart) { }
    public void visit(ClassMethodDecl ClassMethodDecl) { }
    public void visit(NoBrackets NoBrackets) { visit(); }
    public void visit(BracketsB BracketsB) { visit(); }
    public void visit(MulopMod MulopMod) { visit(); }
    public void visit(MulopDiv MulopDiv) { visit(); }
    public void visit(MulopMul MulopMul) { visit(); }
    public void visit(AddopMinus AddopMinus) { visit(); }
    public void visit(AddopPlus AddopPlus) { visit(); }
    public void visit(RelopLessEqual RelopLessEqual) { visit(); }
    public void visit(RelopLess RelopLess) { visit(); }
    public void visit(RelopGreatherEqual RelopGreatherEqual) { visit(); }
    public void visit(RelopGreather RelopGreather) { visit(); }
    public void visit(RelopNotEqual RelopNotEqual) { visit(); }
    public void visit(RelopEqual RelopEqual) { visit(); }
    public void visit(Assignop Assignop) { visit(); }
    public void visit(DesignatorIdentitet DesignatorIdentitet) { visit(); }
    public void visit(DesignatorIdent DesignatorIdent) { visit(); }
    public void visit(DesignatorExpr DesignatorExpr) { visit(); }
    public void visit(NoActualPars NoActualPars) { visit(); }
    public void visit(ActualParamsParen ActualParamsParen) { visit(); }
    public void visit(NoExprBrackets NoExprBrackets) { visit(); }
    public void visit(ExprExists ExprExists) { visit(); }
    public void visit(FactorExpr FactorExpr) { visit(); }
    public void visit(FactorNew FactorNew) { visit(); }
    public void visit(FactorBool FactorBool) { visit(); }
    public void visit(FactorChar FactorChar) { visit(); }
    public void visit(FactorNumber FactorNumber) { visit(); }
    public void visit(FactorDesignator FactorDesignator) { visit(); }
    public void visit(TermFactor TermFactor) { visit(); }
    public void visit(TermMulOpFactor TermMulOpFactor) { visit(); }
    public void visit(SingleTerm SingleTerm) { visit(); }
    public void visit(Terms Terms) { visit(); }
    public void visit(ExprNoMinus ExprNoMinus) { visit(); }
    public void visit(ExprWithMinus ExprWithMinus) { visit(); }
    public void visit(StatementMax StatementMax) { visit(); }
    public void visit(StatementEta StatementEta) { visit(); }
    public void visit(StatementTaraba StatementTaraba) { visit(); }
    public void visit(StatementEt StatementEt) { visit(); }
    public void visit(SimpleExpr SimpleExpr) { visit(); }
    public void visit(MinusAddop MinusAddop) { visit(); }
    public void visit(PlusAddop PlusAddop) { visit(); }
    public void visit(Expr1Term Expr1Term) { visit(); }
    public void visit(ExprAddOp ExprAddOp) { visit(); }
    public void visit(Expr1Minus Expr1Minus) { visit(); }
    public void visit(CondNoRelOp CondNoRelOp) { visit(); }
    public void visit(CondRelOp CondRelOp) { visit(); }
    public void visit(CondFactsMultipleExpr CondFactsMultipleExpr) { visit(); }
    public void visit(CondFactExp CondFactExp) { visit(); }
    public void visit(SingleConditionTerm SingleConditionTerm) { visit(); }
    public void visit(ConditionTerms ConditionTerms) { visit(); }
    public void visit(OrIf OrIf) { visit(); }
    public void visit(SingleCondition SingleCondition) { visit(); }
    public void visit(Conditions Conditions) { visit(); }
    public void visit(ActParsExpr ActParsExpr) { visit(); }
    public void visit(ActParsListExpr ActParsListExpr) { visit(); }
    public void visit(NoActualParameters NoActualParameters) { visit(); }
    public void visit(ActualParameters ActualParameters) { visit(); }
    public void visit(ErrorAssign ErrorAssign) { visit(); }
    public void visit(DesignatorDecOp DesignatorDecOp) { visit(); }
    public void visit(DesignatorIncOp DesignatorIncOp) { visit(); }
    public void visit(DesignatorAssignOp DesignatorAssignOp) { visit(); }
    public void visit(NoExpr NoExpr) { visit(); }
    public void visit(OneExpr OneExpr) { visit(); }
    public void visit(NoElsePart NoElsePart) { visit(); }
    public void visit(ElsePartStm ElsePartStm) { visit(); }
    public void visit(NoNumConst NoNumConst) { visit(); }
    public void visit(OneNumConst OneNumConst) { visit(); }
    public void visit(CaseStatement CaseStatement) { visit(); }
    public void visit(NoCase NoCase) { visit(); }
    public void visit(Cases Cases) { visit(); }
    public void visit(LparenIf LparenIf) { visit(); }
    public void visit(RparenIf RparenIf) { visit(); }
    public void visit(ElseIf ElseIf) { visit(); }
    public void visit(DummyNumb DummyNumb) { visit(); }
    public void visit(StatementGOTO StatementGOTO) { visit(); }
    public void visit(StatementPlusDva StatementPlusDva) { visit(); }
    public void visit(StatementCezar StatementCezar) { visit(); }
    public void visit(StatementSwap StatementSwap) { visit(); }
    public void visit(StatementIf StatementIf) { visit(); }
    public void visit(StatementIfWElse StatementIfWElse) { visit(); }
    public void visit(StatementPrint StatementPrint) { visit(); }
    public void visit(StatementRead StatementRead) { visit(); }
    public void visit(StatementDesignator StatementDesignator) { visit(); }
    public void visit(LabelSingle LabelSingle) { visit(); }
    public void visit(Label Label) { visit(); }
    public void visit(OneLabel OneLabel) { visit(); }
    public void visit(MultLabelList MultLabelList) { visit(); }
    public void visit(NoLabelList NoLabelList) { visit(); }
    public void visit(YesLabelList YesLabelList) { visit(); }
    public void visit(NoStmtList NoStmtList) { visit(); }
    public void visit(StmtList StmtList) { visit(); }
    public void visit(FormalParamDecl FormalParamDecl) { visit(); }
    public void visit(SingleFormalParamDecl SingleFormalParamDecl) { visit(); }
    public void visit(FormalParamDecls FormalParamDecls) { visit(); }
    public void visit(FormPars FormPars) { visit(); }
    public void visit(NoFormParam NoFormParam) { visit(); }
    public void visit(FormParams FormParams) { visit(); }
    public void visit(Type Type) { visit(); }
    public void visit(MehtodTypeNameCustom MehtodTypeNameCustom) { visit(); }
    public void visit(MethodTypeNameVoid MethodTypeNameVoid) { visit(); }
    public void visit(MethodDecl MethodDecl) { visit(); }
    public void visit(NoMethodDeclarations NoMethodDeclarations) { visit(); }
    public void visit(MethodDeclarations MethodDeclarations) { visit(); }
    public void visit(ClassMethodDeclaration ClassMethodDeclaration) { visit(); }
    public void visit(NoVariableDeclarations NoVariableDeclarations) { visit(); }
    public void visit(VariableDeclarations VariableDeclarations) { visit(); }
    public void visit(NoExtendsPart NoExtendsPart) { visit(); }
    public void visit(ExtendsPartType ExtendsPartType) { visit(); }
    public void visit(ClassDecl ClassDecl) { visit(); }
    public void visit(ErrorInDeclaringItem ErrorInDeclaringItem) { visit(); }
    public void visit(ArrayVarPart ArrayVarPart) { visit(); }
    public void visit(VarPartFinal VarPartFinal) { visit(); }
    public void visit(VarPartItem VarPartItem) { visit(); }
    public void visit(VarDeclPart VarDeclPart) { visit(); }
    public void visit(VarDeclarations VarDeclarations) { visit(); }
    public void visit(VarDecl VarDecl) { visit(); }
    public void visit(ConstBool ConstBool) { visit(); }
    public void visit(ConstChar ConstChar) { visit(); }
    public void visit(ConstNumber ConstNumber) { visit(); }
    public void visit(ConstDeclarationPart ConstDeclarationPart) { visit(); }
    public void visit(ConstDeclarations ConstDeclarations) { visit(); }
    public void visit(ConstDecl ConstDecl) { visit(); }
    public void visit(ConstDeclaration ConstDeclaration) { visit(); }
    public void visit(VarDeclaration VarDeclaration) { visit(); }
    public void visit(NoProgramDeclarations NoProgramDeclarations) { visit(); }
    public void visit(ProgramDeclarations ProgramDeclarations) { visit(); }
    public void visit(ProgramID ProgramID) { visit(); }
    public void visit(Program Program) { visit(); }


    public void visit() { }
}
