...
public class SemanticPass extends VisitorAdaptor {
	...
	public static Map<String, Integer> finalVars = new HashMap<>();
	...
	public void visit(ArrayVarPart arrayDeclarationItem) {
		...
		if (addedArray != null) {
			...
			report_info("Dodata niz " + arrayDeclarationItem.getArrayName(), arrayDeclarationItem);
		}
	}
	...
	public void visit(StatementTaraba statementTaraba) {
		statementTaraba.struct = statementTaraba.getDesignator().obj.getType();
	}
	...
	public void visit(ConstChar constChar) {
		...
		if (constNameExists != null) {
			...
			report_info("Dodata konstanta na liniji " + constChar.getLine(), null);
		}
	...
	public void visit(ConstNumber constNumber) {
		...
		if (constNameExists != null) {
			...
			report_info("Dodata konstanta na liniji " + constNumber.getLine(), null);
		}
	...
	public void visit(ConstBool constBool) {
		if (!declType.equals(TabExt.booleanType)) {
			...
			if (constNameExists != null) {
				...
				report_info("Dodata konstanta na liniji " + constBool.getLine(), null);
			}
		}
	}
	...
	public void visit(DesignatorAssignOp designatorAssignOp) {
		...
		if (obj.getKind() == Obj.Var || obj.getKind() == Obj.Elem || obj.getKind() == Obj.Fld) {
			if (designatorAssignOp.getDesignator().obj.getType().compatibleWith(designatorAssignOp.getExpr().struct)) { // ako
				...
				// expr obicni
				// tipovi
				report_info("Designator i expr su odgovarajuceg tipa na liniji " + designatorAssignOp.getLine(),
						designatorAssignOp);
			} else {
				report_error(
	...
	public void visit(CondFactsMultipleExpr condFactsMultipleExpr) {
		...
		if (expr.struct.getKind() == Struct.None) {
			report_error("Prvi expr nije boolean", condFactsMultipleExpr);
		}
		if (expr1.struct.getKind() == Struct.None) {
			report_error("Drugi expr nije boolean", condFactsMultipleExpr);
		}
		...
		if (expr.struct.isRefType() != expr1.struct.isRefType()) {
			...
			if (!(expr1.struct == TabExt.nullType) && !(expr.struct == TabExt.nullType)
				...
				report_error("Nisu kompatibilni tipovi", condFactsMultipleExpr);
			} else if (!(relop instanceof RelopEqual || relop instanceof RelopNotEqual)) {
				report_error("Uz promjenljive tipa niz mogu se samo koristiti samo != i == od relacionih operatora!",
						condFactsMultipleExpr);
			}
		} else {
			if (expr.struct.getKind() != expr1.struct.getKind()) {
				report_error("Nisu kompatibilni tipovi", condFactsMultipleExpr);
			} else if (expr.struct.getKind() == Struct.Bool
					&& !(relop instanceof RelopEqual || relop instanceof RelopNotEqual)) {
				report_error("Uz promjenljive tipa bool mogu se samo koristiti samo != i == od relacionih operatora!",
						condFactsMultipleExpr);
			}
		}
	}
	...
	public void visit(StatementEt statementEt) {
		Obj obj = Tab.find(((DesignatorIdent) statementEt.getDesignator()).getIdentName());
		statementEt.struct = obj.getType().getElemType();
	}
	
	public void visit(StatementEta statementEta) {
//		Obj obj = Tab.find(((DesignatorIdent) statementEta.getDesignator()).getIdentName());
		statementEta.struct = statementEta.getFactor().struct;
	}
	
	public void visit(StatementMax statementMax) {
		statementMax.struct = statementMax.getDesignator().obj.getType().getElemType();
	}
	
	public void visit(VarPartFinal varPartFinal) {
		Obj addedVarPart = TabExt.currentScope.findSymbol(varPartFinal.getVarName());

		if (addedVarPart != null) {
			report_error("Greska: simbol " + varPartFinal.getVarName() + " vec postoji", varPartFinal);
		} else {
			Obj newVarDeclItem = TabExt.insert(Obj.Var, varPartFinal.getVarName(), declType);
			report_info("Dodata promenljiva " + varPartFinal.getVarName(), varPartFinal);
		}
		finalVars.put(varPartFinal.getVarName(), 0);
		report_info("Dodata final promenljiva " + finalVars.get(varPartFinal.getVarName()), varPartFinal);

	}

//	public void visit(StatementIf statementIf) {
//		if(statementIf.getCondition().obj.getType())
//	}
